...
@section('title', 'Редактировать тег - Блог')
    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1 class="h3 mb-0">Редактировать тег: {{ $tag->name }}</h1>
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" id="color-text" value="{{ old('color', $tag->color) }}" readonly>
                                    </div>
                                    @error('color')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="small text-muted">
                                        <div>Статей с тегом: <strong>{{ $tag->articles->count() }}</strong></div>
                                        <div>Создано: <strong>{{ $tag->created_at->format('d.m.Y H:i') }}</strong></div>
                                        <div>Обновлено: <strong>{{ $tag->updated_at->format('d.m.Y H:i') }}</strong></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                <div id="tag-preview" class="text-center">
                                    <span class="badge" id="preview-badge" style="background-color: {{ $tag->color }}; font-size: 1rem; padding: 8px 16px;">
                                        {{ $tag->name }}
                                    </span>
                                </div>
                            </div>
