...
@section('title', 'Редактировать статью - Блог')
...
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
                ...
                <h1 class="h3 mb-0">Редактировать статью: {{ $article->title }}</h1>
                <div>
                    <a href="{{ route('admin.blog.show', $article->id) }}" class="btn btn-outline-info me-2">
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('excerpt') is-invalid @enderror" 
                                              ...
                                              placeholder="Краткое описание статьи для превью...">{{ old('excerpt', $article->excerpt) }}</textarea>
                                    @error('excerpt')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control @error('meta_description') is-invalid @enderror" 
                                              ...
                                              placeholder="Описание для поисковых систем">{{ old('meta_description', $article->meta_description) }}</textarea>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="small text-muted">
                                        <div>Просмотры: <strong>{{ $article->views_count }}</strong></div>
                                        <div>Время чтения: <strong>{{ $article->reading_time ?? 'Не рассчитано' }} мин</strong></div>
                                ...
                                <select class="form-select @error('blog_category_id') is-invalid @enderror" id="blog_category_id" name="blog_category_id">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" 
                                                {{ old('blog_category_id', $article->blog_category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <div id="tags-container" style="max-height: 200px; overflow-y: auto; border: 1px solid #dee2e6; padding: 10px; border-radius: 0.375rem; background-color: #f8f9fa;">
                                    @foreach($tags as $tag)
                                        @php
                                            $isSelected = $article->tags->contains($tag->id);
                                        @endphp
                                        <span class="tag-item {{ $isSelected ? 'selected' : '' }}" 
                                              data-tag-id="{{ $tag->id }}" 
                                              data-tag-name="{{ $tag->name }}"
                                              style="display: inline-block; background: {{ $isSelected ? '#007bff' : '#e9ecef' }}; border: 1px solid #dee2e6; border-radius: 20px; padding: 4px 12px; margin: 2px; font-size: 0.875rem; cursor: pointer; color: {{ $isSelected ? 'white' : 'inherit' }};">
                                            {{ $tag->name }}
                                        </span>
                                    @endforeach
                                </div>
                            ...
                            <div class="card-body">
                                ...
                                <div class="mb-3">
                                    <label for="featured_image" class="form-label">
                                        {{ $article->featured_image ? 'Заменить изображение' : 'Загрузить изображение' }}
                                    </label>
